...
/// The install recorded for a row, if any (only the exact id row shows it;
/// a `replaces` partner being installed elsewhere is reported there).
fn installed_for(row: &Row, pkgs: &PackagesFile, dir: Option<&Path>) -> Option<String> {
    pkgs.pkg(&row.id)?;
    let dir = crate::game_dir::resolve_game_dir(dir).ok()?;
    let state = crate::game_dir::StateFile::load().ok()?;
    let key = crate::game_dir::dir_hash_key(&dir);
    state.install_of(&key, &row.id).map(|i| i.version.clone())
}
...
fn installed_for_display(row: &Row, pkgs: &PackagesFile, dir: Option<&Path>) -> Option<String> {
    pkgs.pkg(&row.id)?;

    let dir = crate::game_dir::resolve_game_dir(dir).ok()?;
    let state = crate::game_dir::StateFile::load().ok()?;
    let key = crate::game_dir::dir_hash_key(&dir);
    let inst = state.install_of(&key, &row.id)?;
    let chk = crate::game_dir::check_install(&dir, inst);

    // All files gone (and none moved elsewhere) - treat as not installed.
    if chk.fully_gone() {
        return None;
    }
    let version = packages::list_version(&inst.version);
    Some(if chk.all_present() {
        version.to_string()
    } else if version.is_empty() {
        "(some files missing)".to_string()
    } else {
        format!("{version}   (some files missing)")
    })
}
...
fn narrow_rows(
    ...
    if let Some(exact) = all.iter().find(|r| match_names::normalize(&r.id) == norm) {
        return Ok(vec![exact.clone()]);
    }

    let hits = match_names::resolve(pkgs, name, game).map_err(ChefError::Other)?;
    if let Some(g) = game
    ...
    let narrowed = match_names::narrow_by_game(hits, pkgs, game);
...
pub fn run(
    ...
    let dir = crate::game_dir::resolve_game_dir(dir.as_deref()).ok();
        ...
        let entries = rows
            ...
            .map(|r| {
                ...
                json!({
                    ...
                    "latest": latest,
                    "preview": preview,
                    "installed": installed_for(r, &pkgs, dir.as_deref()),
                })
            })
            .collect::<Vec<_>>();
    ...
    let w_avail = rows
        ...
        .unwrap_or(9)
        .max(9);
    println!("{:<w_title$} {:<w_avail$} INSTALLED", "TITLE", "AVAILABLE");

    for r in &rows {
        let entries = packages::available_versions(&pkgs, &lock, &r.id, detected_game.as_deref());
        let avail = avail_texts(&entries).join(", ");
        let inst = installed_for_display(r, &pkgs, dir.as_deref()).unwrap_or_else(|| "-".into());
        println!("{:<w_title$} {:<w_avail$} {}", r.name, avail, inst);
    }
    Ok(())
}
