...
use crate::packages::{self, LockFile, PackagesFile, ResolvedVersion};
use crate::utils::picker;

pub fn split_pkg_spec(input: &str) -> (&str, Option<&str>) {
...
pub(crate) fn install_one(
    ...
    if dry_run {
        ...
        let files = if store_complete(product.store_key(), &product.version) {
            let vdir = crate::store::ensure_payload(
                product.store_key(),
                &product.name,
                &product.version,
                &product.url,
...
pub fn run(
    ...
    for input in inputs {
        let (name, spec) = split_pkg_spec(input);
    ...
    if json {
        crate::emit_json(&serde_json::json!({ "add": rows }));
        return Ok(());
    }
