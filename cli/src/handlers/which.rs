...
/// Report installed packages. `pkg` is either nothing (summary of every
/// package in the game dir) or an already-split `(name, spec)`. `game_dir`
/// is the resolved target.
pub fn run(pkg: Option<(&str, Option<&str>)>, game_dir: &Path, json: bool) -> crate::Result<()> {
