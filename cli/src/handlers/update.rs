...
use super::add as add_cmd;

/// Update installed packages to the newest stable release: the requested
/// one, or everything installed in `game_dir`. Reports (and applies) only
/// packages with a newer stable release.
pub fn run(pkg: Option<&str>, game_dir: &Path, dry_run: bool, json: bool) -> crate::Result<()> {
    ...
    let targets: Vec<String> = match pkg {
        Some(name) => {
            ...
            if let Some(p) = pkgs
                .packages
                .iter()
                .find(|p| packages::normalize(&p.id) == norm)
            {
                vec![p.id.clone()]
            } else {
                vec![packages::resolve_id(&pkgs, name, game.as_deref())?]
            }
        }
        None => state
            .installs_in(&key)
    ...
    if targets.is_empty() {
        if json {
            crate::emit::emit_json(&serde_json::json!({ "update": [], "dryRun": dry_run }));
            return Ok(());
        }
    ...
    if json {
        crate::emit::emit_json(&serde_json::json!({ "update": rows, "dryRun": dry_run }));
        return Ok(());
    }
